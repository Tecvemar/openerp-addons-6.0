                ...
                for fn in field_names:
                    ...
                    for child in current.child_id:
                        if child.company_id.currency_id.id == current.company_id.currency_id.id:
                            ...
                            sums[current.id][fn] += sums.get(child.id, {fn: 0.0})[fn]
                        else:
...
class account_move(osv.osv):
    ...
    def validate(self, cr, uid, ids, context=None):
        ...
        for move in self.browse(cr, uid, ids, context):
            # Unlink old analytic lines on move_lines (moved inside next "for")
            unlink_analytic_ids = []
            ...
            for line in move.line_id:
                for obj in line.analytic_lines:
                    unlink_analytic_ids.append(obj.id)
                amount += line.debit - line.credit
            ...
            if unlink_analytic_ids:
                obj_analytic_line.unlink(cr, uid, unlink_analytic_ids)
